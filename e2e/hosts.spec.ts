...
test.describe('Hosts', () => {
  test('should display hosts page with list of hosts', async ({ authenticatedPage }) => {
    ...
    const hasEmptyState = await page
      .getByText(/no hosts have reported yet/i)
      .isVisible()
      .catch(() => false);
  ...
  test('should filter hosts by major version when distribution is selected', async ({
    authenticatedPage,
  }) => {
    const page = authenticatedPage;
    await page.goto('/hosts');
    ...
    if (specificDistro) {
      ...
      if (isMajorVersionVisible) {
        ...
        if (majorVersionOptions.length > 1) {
          ...
          const specificMajorVersion = majorVersionOptions.find(
            opt => opt.toLowerCase() !== 'all major versions'
          );
  ...
  test('should display host detail page correctly', async ({ authenticatedPage }) => {
    ...
    if (rowCount > 0) {
      ...
      const hasSystemInfo = await page
        .getByText(/system|hostname|os/i)
        .first()
        .isVisible()
        .catch(() => false);
  ...
  test('should navigate back to hosts list from host detail', async ({ authenticatedPage }) => {
    ...
    if (rowCount > 0) {
      ...
      const backButton = page
        ...
        .or(page.getByText(/hosts/i).first());
  ...
  test('should display empty state message when no hosts match filter', async ({
    authenticatedPage,
  }) => {
    const page = authenticatedPage;
    await page.goto('/hosts');
