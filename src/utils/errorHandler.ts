  ...
  getStats() {
    ...
    return {
      ...
      byCategory: Object.values(ErrorCategory).reduce((acc, category) => {
        acc[category] = recentLogs.filter(log => (log.context as any)?.category === category).length
        return acc
...
export const handleNetworkError = (error: Error, url?: string, method?: string, status?: number, response?: any) =>
