...
function generateRecommendations(analysis: BundleAnalysis): string[] {
  ...
  if (analysis.totalSize > 2000 * 1024) {
    // > 2MB
    recommendations.push(
...
export class BundlePerformanceMonitor {
   ...
   */
  monitorLazyLoad<T>(componentName: string, importFn: () => Promise<T>): Promise<T> {
...
export function monitoredLazy<T extends React.ComponentType<any>>(
  ...
  return React.lazy(() => bundleMonitor.monitorLazyLoad(componentName, importFn));
}
