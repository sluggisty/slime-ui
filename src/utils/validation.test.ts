...
describe('Validation Utilities', () => {
  ...
  describe('Error Handling', () => {
    describe('getSafeErrorMessage', () => {
      it('returns user-friendly error messages', () => {
        expect(getSafeErrorMessage('SQL error')).toBe('A system error occurred. Please try again later.')
  ...
  describe('Schema Validation', () => {
    describe('validateAgainstSchema', () => {
      const schema = {
        username: [
          { validate: (v: any) => typeof v === 'string' && v.length >= 3, message: 'Username too short' },
          { validate: (v: any) => typeof v === 'string' && v.length <= 20, message: 'Username too long' }
        ],
        email: [
          { validate: (v: any) => typeof v === 'string' && v.includes('@'), message: 'Invalid email' }
        ]
      }
