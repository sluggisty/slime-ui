    ...
    <div className={styles.page}>
      ...
      {errors && errors.length > 0 && (
        <Card className={styles.errorsCard}>
          <h3>
            <AlertTriangle size={18} /> Collection Errors
          </h3>
          ...
          <CollapsibleSection title='Hardware' icon={<Cpu size={18} />}>
            <div className={styles.dataGrid}>
              ...
              <DataItem
                label='Memory Used'
                value={
                  hardware.memory?.percent_used
                    ? `${hardware.memory.percent_used.toFixed(1)}%`
                    : undefined
                }
              />
              {hardware.cpu?.load_average && (
                <>
                  <DataItem
                    label='Load (1min)'
                    value={hardware.cpu.load_average['1min']?.toFixed(2)}
                  />
                  <DataItem
                    label='Load (5min)'
                    value={hardware.cpu.load_average['5min']?.toFixed(2)}
                  />
                  <DataItem
                    label='Load (15min)'
                    value={hardware.cpu.load_average['15min']?.toFixed(2)}
                  />
                </>
              )}
            </div>
              ...
              <div className={styles.failedServices}>
                ...
                <ul className={styles.serviceList}>
                  {services.failed_units.map((unit, i) => (
                    <li key={i} className={styles.failed}>
                      {unit.name}
                    </li>
                  ))}
                </ul>
                ...
                <div key={i} className={styles.mount}>
                  <div className={styles.mountHeader}>
                    ...
                    {mount.percent_used !== undefined && (
                      <Badge
                        variant={
                          mount.percent_used > 90
                            ? 'error'
                            : mount.percent_used > 80
                              ? 'warning'
                              : 'success'
                        }
                      >
          ...
          <CollapsibleSection title='Security' icon={<Shield size={18} />}>
            <div className={styles.dataGrid}>
              <DataItem
                label='SELinux'
                value={
                  security.selinux?.enabled ? `${security.selinux.mode || 'enabled'}` : 'Disabled'
                }
              />
              <DataItem label='FIPS' value={security.fips?.enabled ? 'Enabled' : 'Disabled'} />
            </div>
          </CollapsibleSection>
        )}
      </div>
