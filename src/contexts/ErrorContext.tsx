...
export const ErrorProvider: React.FC<ErrorProviderProps> = ({
  ...
  const addNetworkError = React.useCallback(
    async (error: Error, url?: string, method?: string, status?: number) => {
      dispatch({ type: 'SET_LOADING', payload: true });
    ...
    >
      ...
      >
        {message.severity === ErrorSeverity.CRITICAL
          ? '🚨'
          : message.severity === ErrorSeverity.HIGH
            ? '⚠️'
            : message.severity === ErrorSeverity.MEDIUM
              ? 'ℹ️'
              : '💡'}
      </div>
      ...
      <div style={{ flex: 1 }}>
        ...
        <p style={{ margin: '0 0 8px 0', fontSize: '14px', color: '#666' }}>{message.message}</p>
        {message.action && (
          <button
            style={{
