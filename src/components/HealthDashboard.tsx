...
import { useComponentPerformance, useInteractionTracking } from '../hooks/usePerformanceMonitoring';
...
export const HealthDashboard: React.FC = () => {
  ...
  const { measureAsync } = useComponentPerformance('HealthDashboard');
      ...
      <div className={`${styles.overallStatus} ${getOverallStatusColor(healthStatus.overall)}`}>
        ...
        <div className={styles.statusInfo}>
          ...
          <p className={styles.statusDescription}>
            {Object.keys(healthStatus.checks).length} checks • {failedChecks} failed •{' '}
        ...
        {Object.entries(healthStatus.checks).map(([name, check]) => (
          <div key={name} className={styles.checkCard}>
            <div className={styles.checkHeader}>
              <div className={styles.checkIcon}>{getCheckIcon(name)}</div>
              <div className={styles.checkInfo}>
                <h4 className={styles.checkName}>{name}</h4>
                <div className={styles.checkStatus}>
                  {getStatusIcon(check.status)}
                  <span className={styles.checkStatusText}>{getStatusText(check.status)}</span>
                </div>
              </div>
            </div>
            ...
            <div className={styles.checkDetails}>
              <div className={styles.checkMetric}>
                ...
                <span className={styles.metricValue}>{Math.round(check.duration)}ms</span>
              </div>
            ...
            <button onClick={() => handleViewDetails(name)} className={styles.viewDetailsButton}>
              View Details
            </button>
          </div>
        ))}
      ...
      <div className={styles.performanceSummary}>
        ...
        <div className={styles.performanceGrid}>
          ...
          <div className={styles.performanceItem}>
            ...
            <span className={styles.performanceValue}>
              {Math.round(
                (Object.values(healthStatus.checks).filter(check => check.status === 'pass')
                  .length /
                  Object.keys(healthStatus.checks).length) *
                  100
              )}
