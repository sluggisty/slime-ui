...
function outputCICDReport(report) {
  ...
  console.log(
    `📦 **Total Size**: ${formatBytes(build.totalSize)} (${formatBytes(build.totalSizeGzip)} gzipped)`
  );
  ...
  budgets.forEach(budget => {
    ...
    console.log(
      `${status} ${budget.name}: ${formatBytes(budget.size)} / ${formatBytes(budget.budget)} (${percentage}%)`
    );
