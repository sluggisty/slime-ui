...
function analyzeBundle() {
  ...
  if (!fs.existsSync(assetsDir)) {
    console.log(`${colors.red}❌ No dist/assets directory found. Run 'npm run build' first.${colors.reset}`)
    return
  }
...
function generateRecommendations(chunks, totalSize) {
  ...
  const hasDynamicImports = chunks.some(chunk => chunk.name.includes('lazy') || chunk.name.includes('async'))
